...
class PTPController:
    ...
    def __init__(self, group, cache, coord_system):
        ...
        # 优化的规划参数（针对Gazebo仿真）
        self.group.set_planning_time(5.0)  # 增加规划时间
        self.group.set_planner_id("RRTConnect")
        self.group.set_num_planning_attempts(3)  # 增加尝试次数
        self.group.set_max_velocity_scaling_factor(0.3)  # 降低速度
        self.group.set_max_acceleration_scaling_factor(0.3)  # 降低加速度
        self.group.set_goal_joint_tolerance(0.01)  # 放宽容差
...
def control_gripper(action="open"):
    """控制夹爪（适配Gazebo仿真）"""
    try:
        # 在Gazebo中使用panda_hand组
        gripper_group = MoveGroupCommander("panda_hand")
        gripper_group.set_planning_time(2.0)  # 增加规划时间
        gripper_group.set_max_velocity_scaling_factor(0.5)  # 降低速度
        ...
        if action == "open":
            ...
            # Gazebo中的夹爪关闭值
            gripper_group.set_joint_value_target([0.025, 0.025])
        ...
        # 在仿真中等待更长时间确保夹爪动作完成
        rospy.sleep(1.0 if action == "close" else 0.5)
        
        print(f"夹爪{action}: {'成功' if success else '失败'}")
        return success
    except Exception as e:
        print(f"警告: 无法{action}夹爪: {e}")
        return False
...
def pick_object(ptp_controller, group, scene, object_name, coord_system):
    ...
    # 5. 等待夹爪完全关闭并稳定
    print("等待夹爪稳定...")
    rospy.sleep(2.0)  # 增加等待时间确保夹爪稳定
    
    # 6. 提升（先移动再附着）
    retreat_pose = coord_system.get_retreat_pose(target_pos)
    if not ptp_controller.move_to_pose_ptp(retreat_pose):
        print(f"✗ 提升失败")
        return False
...
def place_object(ptp_controller, group, scene, object_name, coord_system):
    ...
    # 2. 下降到放置位置
    place_pos = [target_pos[0], target_pos[1], target_pos[2] + 0.13]
...
def main():
    print("\nPanda机器人Gazebo仿真PTP控制多物体搬运系统\n")
    
    # 初始化ROS和MoveIt
    moveit_commander.roscpp_initialize(sys.argv)
    rospy.init_node("panda_gazebo_pick_place", anonymous=True)
    ...
    # 打印结果
    print(f"\nGazebo仿真搬运完成！成功率: {successful_count}/{len(object_names)} ({successful_count*100/len(object_names):.1f}%)")
